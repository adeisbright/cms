 ...
 */
let userIdentifier = Array.from(selectAll(".name"))
...
let productButton = selector("#productButton")
let selectField  = selector("productCategory") 
let generator    = selector(".generator") 
let spareParts   = selector(".spare-parts")
...
class RegistrationController {
    ...
    handleClick(event) {
        if (event.target.id === "deleteButton"){
            ...
            sendData(`/admin/categories/delete`  , {categories})
            .then(res => {
                Array.from(selectAll(".check")).map((e , i) => {
                    if (e.checked){ 
        ...
        if (event.target.id === "productButton"){
            let products = [] 
            Array.from(selectAll(".check")).map((e , i) => {
                if (e.checked){ 
                    let email = userIdentifier[i].getAttribute("data-identifier")
            ...
            sendData(`/admin/products/delete`  , {products})
            .then(res => {
                Array.from(selectAll(".check")).map((e , i) => {
                    if (e.checked){ 
                        e.parentNode.parentNode.remove() 
    ...
    handleSelect(event) {
        const otherItems = ["solar"  ,"accessories" ]
        if (event.target.id === "productCategory") {
            let value  = event.target.value 
            if (value.toLowerCase().trim().substr(0 , 5) === "gener"){
                generator.style.display = "block"
                spareParts.style.display = "none"
            }else if (value.toLowerCase().trim().substr(0 , 5) === "spare"){
                spareParts.style.display = "block"
                generator.style.display = "none"
            }else if (otherItems.includes(value.toLowerCase().trim() )){
                spareParts.style.display = "none"
                generator.style.display = "none"
                description.style.display = "block"
            } 
            let r = Array.from(selector("#productCategory").childNodes) 
            console.log(r)
        }
    ...
    handleSubmit(event) { 
		if (event.target.id === "submitProduct1") {   
            ...
            let allInput = Array.from(selectAll(".form-control")) 
            allInput.map(e => console.log(e.value)) 
            console.log("Hi")
    ...
    addEvent() {
        ...
        if (productButton) productButton.addEventListener("click" , this.handleClick)
        if (blockButton) blockButton.addEventListener("click" , this.handleClick)
        if (unBlockButton) unBlockButton.addEventListener("click" , this.handleClick)
        if (selector(".generator")) {
            window.addEventListener("load" , e => {
                //selector(".generator").style.display = "none"
                selector(".spare-parts").style.display = "none"
                selector(".description").style.display = "none"
            })
        }
    }
} 
...
let modalContent = selector(".modal-content") 
