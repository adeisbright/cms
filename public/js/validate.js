...
export const validateMobile = (val) => { 
	...
	const thirdPattern = /^[8]{1}[0|1]{1}[0-9]{8}$/
    const fourthPattern = /^[7 | 9]{1}[0]{1}[0-9]{8}$/ 
	try {
		if ( String(val).match(firstPattern) || String(val).match(secondPattern) || String(val).match(thirdPattern) || String(val).match(fourthPattern)) {
	        return {
		        name : "Matched" , 
		        value : val.trim() 
	        } 
...
/**
 * @description    validateEmail() as a function validates input field created for email inputs
 * @param  {String} A string returned from the input field 
 * @return {Object} An object specifying a message an a value after validation
*/
